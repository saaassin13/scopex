...
输出以时间+现象、值/形态/证据为主；数据是否出现异常形态与设备故障归因分开。需要判断动作是否符合指令时必须有独立控制或业务状态，缺证据不是故障，也不能据此证明正常，应保留需复核结论。只有`rebound_supported=true`才能称形态支持回弹；过程数、候选数或幅度不能脱离形态与上下文直接当故障数。
