...
class BudgetCoordinator:
    def __init__(self, task, session, events, audit, *, reason, with_evidence):
        self.task = task
    ...
    def start(self, message, *, turn_name):
        ...
        if self.with_evidence:
            self.catalog.add(
                source="/agent-data/sample.log",
                raw="observed fact before budget stop",
        ...
        return OpenClawTurnResult(
            ...
            audit_dir=root,
            runtime_limit_reason=self.reason,
        )

    def begin_runtime_limit_finalization(self, reason):
        self.controller.begin_finalization(reasons=("budget_reached", reason))
        ...
        self.audit.persist_result(
            {
                "valid": True,
                "errors": [],
                "investigation_reasons": ["budget_reached", self.reason],
                "task_state": TaskState.COMPLETED.value,
            },
            rendered="bounded partial result",
        )
...
class BudgetFactory:
    def __init__(self, *, reason="model_request_budget", with_evidence=True):
        self.reason = reason
        self.with_evidence = with_evidence
...
class BudgetServiceTests(unittest.TestCase):
    def test_budget_stop_with_evidence_finalizes_current_facts(self):
        with tempfile.TemporaryDirectory() as td:
            service = TaskService(
                audit_root=Path(td),
                coordinator_factory=BudgetFactory(with_evidence=True),
                finalizer_factory=lambda: object(),
            )
            task = service.create_task("diagnose until bounded")
            ...
            self.assertEqual(
                ...
                ["budget_reached", "model_request_budget"],
            )
            runtime_limit = service.store.read_json(task["id"], "runtime-limit.json")
            self.assertEqual(runtime_limit["reason"], "model_request_budget")
            events = service.get_events(task["id"])
            self.assertTrue(any(
                row["type"] == "INVESTIGATION_COMPLETED"
                and row.get("data", {}).get("reasons")
                == ["budget_reached", "model_request_budget"]
                for row in events
            ))

    def test_budget_stop_without_evidence_fails_cleanly(self):
        with tempfile.TemporaryDirectory() as td:
            service = TaskService(
                audit_root=Path(td),
                coordinator_factory=BudgetFactory(with_evidence=False),
                finalizer_factory=lambda: object(),
            )
            task = service.create_task("no evidence before limit")
            terminal = wait_terminal(service, task["id"])
            self.assertEqual(terminal["state"], "FAILED")
            self.assertEqual(terminal["last_reason"], "budget_reached_without_evidence")
